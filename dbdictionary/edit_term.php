...
<body>
  ...
  <script>alert("ghghg");</script>
	...
	<form method="post" action="edit_term.php">
...
<td>$id</td>
			...
			<td>$trans</td>
			<td>$defe</td>
إ			<td>  <input name='termp' type='image' src='$picture' width='80' height='80'  /></td>
	...
	<form method='post' action='edit_term.php?id=$_GET[id]&amp;action=$_GET[action]' enctype="multipart/form-data">
	...
	if(!is_dir('pic')){ mkdir('pic');     }
		...
		if(!empty($fileName)){  move_uploaded_file($tmpName,'pic/'.$fileName);    }
